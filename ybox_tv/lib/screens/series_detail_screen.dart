...
class _SeriesDetailScreenState extends State<SeriesDetailScreen> {
  ...
  void _play(Episode ep) {
    // Pass the whole series (all seasons, in order) so the player shows the
    // Next / Previous episode buttons and can auto-roll into the next one.
    final all = [
      for (final s in _info?.seasons ?? const <Season>[])
        ...s.episodes,
    ];
    final idx = all.indexWhere((e) => e.id == ep.id);
    PlayerScreen.open(
      context,
      widget.series,
      url: ep.streamUrl,
      title: '${widget.series.name} · S${ep.season} E${ep.episodeNum}',
      episodes: all.isEmpty ? null : all,
      episodeIndex: idx < 0 ? null : idx,
    );
  }
